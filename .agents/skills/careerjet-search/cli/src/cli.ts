#!/usr/bin/env bun
// Self-contained CLI for searching the Careerjet public API
// (https://www.careerjet.com/partners/api/). No authentication, no external
...
const HELP = `careerjet-search-cli — search the Careerjet public API (locales: it_IT, es_ES)
...
SEARCH FLAGS
  ...
  --country <code>        Careerjet locale: it (default, it_IT) | es (es_ES).
  --page <n>              1-indexed page. Default 1.
  --limit, -n <n>         Results per page (API max 99). Default 25.
  --format <fmt>          json (default) | table | plain.

EXAMPLES
  bun run src/cli.ts search -q "responsabile it" -l "Bologna" --country it --format table
  bun run src/cli.ts search -q "AI consultant" --country es --limit 10 --format json

Auth: none. The Careerjet public API is free and keyless; only a polite
User-Agent header is sent. There is no separate detail command.
`
...
export async function main(argv = process.argv.slice(2), emitError: ErrorWriter = writeError): Promise<number> {
  try {
    ...
    const opts: SearchOpts = {
      ...
      limit: integerFlag("limit", stringFlag(flags, "limit") ?? "25", 1, 99),
      format: fmt,
    }
