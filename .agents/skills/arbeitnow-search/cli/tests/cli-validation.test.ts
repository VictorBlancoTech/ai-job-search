...
describe("CLI validation", () => {
  test("rejects malformed usage with exit 2 and INVALID_ARGUMENT", async () => {
    ...
    expect(negativeLimit.error.error).toContain("range 1..100")
    expect(negativeLimit.error.error).not.toContain("requires a value")
  })

  test("parses remote-only as a boolean flag without consuming a value", () => {
    const flags = parseArgs(["search", "--remote-only"])
    expect(flags["remote-only"]).toBe(true)
  })
})
